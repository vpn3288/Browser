...
.NOTES
    ...
    Version: 14.1 - 最终版：允许登录、导入、修复CF验证、保留核心反检测
    Date: 2026-05-17 (Final)
#>
...
function Set-ChromiumAdvancedConfig {
    ...
    $policies = @{
        ...
        # 安全
        "SSLErrorOverrideAllowed" = 1  # v14.1: 允许SSL错误覆盖
    ...
    if ($BrowserKey -eq "Edge") {
        ...
        # Edge WebRTC 使用不同的策略名
        $policies["WebRtcLocalhostIpHandling"] = "disable_non_proxied_udp"  # v14.1: Edge专用WebRTC策略
    }
...
// ===== 核心反指纹（可选，注释掉以提升兼容性）=====
// user_pref("privacy.resistFingerprinting", true);  // 可选：强隐私模式，会改变时区和部分网页行为
...
// ===== WebRTC IP防护（不完全禁用）=====
user_pref("media.peerconnection.enabled", true);  // v14.1: 启用WebRTC（某些网站需要）
user_pref("media.peerconnection.ice.default_address_only", true);  // 但防止IP泄露
user_pref("media.peerconnection.ice.no_host", true);
user_pref("media.peerconnection.ice.proxy_only_if_behind_proxy", true);

// ===== 书签在新标签页打开 =====
user_pref("browser.tabs.loadBookmarksInTabs", true);  // v14.1: 书签在新标签页打开
user_pref("browser.tabs.loadBookmarksInBackground", false);  // v14.1: 并跳转到新标签页
...
// ===== DNS-over-HTTPS =====
user_pref("network.trr.mode", 2);  // v14.1: 2 = TRR first with fallback（更稳定）
