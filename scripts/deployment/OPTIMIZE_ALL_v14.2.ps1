...
.NOTES
    ...
    Version: 14.2 - 热修复：删除启动器负优化、修复策略错误
...
function Set-ChromiumAdvancedConfig {
    ...
    if ($BrowserKey -eq "Brave") {
        ...
        $policies["BraveNewsEnabled"] = 0  # v14.2: 禁用Brave News
    }
    ...
    if ($BrowserKey -eq "Edge") {
        ...
        # Edge也支持通用WebRtcIPHandlingPolicy，无需单独配置
    }
...
Write-Host "  多浏览器反检测优化工具 v14.2" -ForegroundColor Green
