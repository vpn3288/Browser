...
<#
.SYNOPSIS
    Multi-Browser Anti-Detect Optimization Tool v14.5
.DESCRIPTION
...
.NOTES
    ...
    Version: 14.5 - 最终修复：删除负优化、修正策略名：删除启动器、修正策略名、清理冗余
...
$languageConfig = @{
    ...
    "Firefox" = "en-US"
    "LibreWolf" = "en-US"
    "Zen Browser" = "en-US"  # v14.5: 全部使用en-US（正常美国人）
}
...
function Set-ChromiumAdvancedConfig {
    ...
    $policies = @{
        ...
        # 反检测核心
        # v14.5: 删除 - 虚假优化，暴露浏览器被修改
        # v14.5: 删除 - 虚假优化，暴露浏览器被修改
        # v14.5: WebRTC策略 - Edge使用专用策略名
    ...
    if ($BrowserKey -eq "Edge") {
        ...
        $policies["WebRtcLocalhostIpHandling"] = "disable_non_proxied_udp"  # v14.5: Edge专用WebRTC策略
    }
...
Write-Host "  多浏览器反检测优化工具 v14.5" -ForegroundColor Green
